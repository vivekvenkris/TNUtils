...
parser.add_argument('-p','--plot', help='get the subset of params to plot', const='', default='', nargs='?')
parser.add_argument('-d','--device', help='say live or give a file name', const='live', default='live')
parser.add_argument('-s', '--rescale', action='store_true')
parser.add_argument('-r','--results', help='directory of results', const='', default='')
parser.add_argument('-f','--fig_size', help='PAGE OR COLUMN OR GROW', const='GROW', default='GROW')
parser.add_argument('-n','--psr_name', help='J or B name of psr', const='', default='', nargs='?', required=True)
...
try:
	...
	if(len(dir_contents) == 0:
        ...
        col_names = open(filename).read().splitlines()
        ...
        if(args.plot != ""):
        	print("shortlisting requested parameters:" + args.plot)
        	required = args.plot.strip().split(" ").split(",")
	    ...
	    if args.rescale:
            ...
            for line in open(scaling_file, 'rb').read().splitlines():
